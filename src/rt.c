 ...
 */
 ...
 #include "amiga.h"
 #include <clib/compiler-specific.h> /* for interrupt and register macros */
 #include <sys/timer.h>
 #include <sys/time.h>
 #include <errno.h>
 #include <string.h>
...
extern void *memmove(void *dest, const void *src, size_t n);
 ...
 static void __SAVE_DS__ __INTERRUPT__ _timer_handler_func(struct amiga_timer *timer);
 ...
 static void __SAVE_DS__ __INTERRUPT__ _timer_handler_func(struct amiga_timer *timer) {
  ...
  int timer_create(clockid_t clockid, struct sigevent *sevp, timer_t *timerid) {
     ...
     timer = &timers[timer_count];
      ...
      timer->port->mp_SigTask = (struct Task *)&timer->handler;
