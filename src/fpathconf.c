...
/*
 * get_filesystem_limits_by_fd() - Get filesystem limits for a specific file descriptor
 *
 * This function determines the actual filename and path length limits
 * for a filesystem by examining the device associated with the file descriptor.
 *
 * Parameters:
 *   fd - File descriptor to examine
 ...
 */
static int get_filesystem_limits_by_fd(int fd, long *name_max, long *path_max)
{
...
long fpathconf(int fd, int name)
{
    ...
    switch (name) {
        ...
        case _PC_NAME_MAX: {
            ...
            if (get_filesystem_limits_by_fd(fd, &name_max, &path_max) == 0) {
                return name_max;
            }
